...
Sepolia, and receives a receipt. The placement right is bought at a price both sides agreed in advance,
and the receipt records exactly what was purchased.
...
```
  ...
  or curator UI  │                              │
              ...
              (config + store)      resource server
                         ...
                         │              │
                         ▼              ▼
                 receipt record   USDC on Base Sepolia
                                  (eip155:84532)
```
...
| Payments | `lib/x402.ts` | x402 resource server, facilitator client, EVM exact scheme, settlement hook. |
...
may be sent in the query string or the body; for paid requests the query string is authoritative,
because the price is settled before the body is read.
...
No token. No AMM. No mainnet deployment. No scraping of Spotify. No stream-count or royalty claims —
